...
Use incast_whiskers.py on the directory contianing xml files
copy to cloud bucket with 
`sudo gsutil -m cp -r *.png gs://homa-pbs/Incast_Debug_V2/`
*/
...
//
    ...
    if (profile != 6) {
        cout << "ERROR not supported workload profile only for incast (6)\n";
        return -1;
    }
	...
	switch (profile)
	{
		...
		case 6: // incast
			...
			tag = "incast_" + tag;
			sim_duration = 10000000; //ten thousand seconds 
			break;
	}
...
//
	...
	std::cout << "Number of incast flows = " << incast_servers << "\n";
	cout << "Total simulation time = " << sim_duration << " s\n";
...
//
	...
	for (i = 0; i < incast_servers; i++)
	{
		...
		src_host++;
		if (src_host >= num_host) {
			src_edge++;
			src_host = 0;
		}
		if (src_edge >= num_edge) {
			src_pod++;
			src_edge = 0;
		}
		if (src_pod >= num_pod) {
			cout << "ERROR to many senders\n";
			return -1;
		}
	}
