...
def main(argv):
	...
	# DO NOT INCLUDE ACKs
        ...
        dctcp_data.append( np.array(fcts).reshape((-1,1)) )
	...
	# DO NOT INCLUDE ACKs
        ...
        for flow in flows_of_interest:
            if flow.rawDuration > 0:
                fcts.append(float(flow.rawDuration))
        pbs_data.append( np.array(fcts).reshape((-1,1)) )
    ...
    outfilename = "pbs_fct_fairness.png"
    plt.tight_layout()
    ...
    plt.xlabel("Number of Incast Senders")
    plt.xticks([k for k in range(1,129)], (str(k) if k%16 == 0 else "" for k in range(1,129)), rotation=20)
    outfilename = "dctcp_fct_fairness.png"
    plt.tight_layout()
    plt.savefig(outfilename)
    plt.clf()
