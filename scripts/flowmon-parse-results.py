...
class Flow(object):
    ...
    def __init__(self, flow_el):
        ...
        actual_flow_duration = float(long(flow_el.get('timeLastRxPacket')[:-4]) - long(flow_el.get('timeFirstTxPacket')[:-4]))*1e-9
        print "Actual Duration: ", actual_flow_duration
        optimal_flow_duration = long(flow_el.get('txBytes'))*8.0 / BOTTLENECK_RATE
        print "Optimal Duration: ", optimal_flow_duration
        #slowdown = math.log( actual_flow_duration / optimal_flow_duration, 10 )
        slowdown = actual_flow_duration / optimal_flow_duration
