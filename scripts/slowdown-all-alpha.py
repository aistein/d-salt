...
def main(argv):
	...
	# DO NOT INCLUDE ACKs
        ...
        buckets = np.logspace(2, np.log10(max(flowsizes)), num=10, base=10, dtype=long)
        print "buckets = ", buckets
        # Buckets of Slowdown organized by Size
        slowdown_by_bucket = defaultdict(list)
        for flow in flows_of_interest:
            for fsize in buckets:
                if flow.size <= fsize:
                    slowdown_by_bucket[fsize].append(flow.slowdown)
                    break
        # 99th Percentile Slowdown for each Bucket
        sldns_99p = defaultdict(float)
        for fsize, sldns in slowdown_by_bucket.items():
           sldns_99p[fsize] = np.percentile(sorted(sldns), 99) 

	flowSizes99 = [fsize/8 for fsize,_ in sorted(sldns_99p.items())]
        slowdowns99 = [sldn for _,sldn in sorted(sldns_99p.items())]
        plt.plot(flowSizes99, slowdowns99, colors[i], linestyle='-', label=sim_names[i]+"_99p")

    plt.title(argv[1].split("/")[2].split(".")[0].split("_")[0] + " - Flow Size vs. 99% Slowdown")
    plt.xlabel("Flow Size (bytes)")
    plt.ylabel("Slowdown")
    plt.yscale('log')
    plt.xscale('log')
    plt.legend()
    outfilename = argv[1].split("/")[2].split(".")[0].split("_")[0] + "_sldn_99p_all.png"
        ...
        for flow in flows_of_interest:
            for fsize in buckets:
                if flow.size <= fsize:
                    slowdown_by_bucket[fsize].append(flow.slowdown)
                    break
        # 50th Percentile Slowdown for each Bucket
        sldns_50p = defaultdict(float)
        for fsize, sldns in slowdown_by_bucket.items():
           sldns_50p[fsize] = np.percentile(sorted(sldns), 50) 

	flowSizes50 = [fsize/8 for fsize,_ in sorted(sldns_50p.items())]
        slowdowns50= [sldn for _,sldn in sorted(sldns_50p.items())]
        plt.plot(flowSizes50, slowdowns50, colors[i], linestyle='-', label=sim_names[i]+"_50p")

    plt.title(argv[1].split("/")[2].split(".")[0].split("_")[0] + " - Flow Size vs. Median Slowdown")
    plt.xlabel("Flow Size (bytes)")
    plt.ylabel("Slowdown")
    plt.yscale('log')
    plt.xscale('log')
    plt.legend()
    outfilename = argv[1].split("/")[2].split(".")[0].split("_")[0] + "_sldn_50p_all.png"
    plt.grid(True, which="both")
    plt.tight_layout()
    plt.savefig(outfilename)
    plt.clf()
