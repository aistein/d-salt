...
//
	...
	cmd.AddValue("useDctcp", "switch to toggle DCTCP mode (bool)", use_pbs);
	cmd.Parse (argc, argv);
...
//	
	...
	p2p_hostToEdge.SetDeviceAttribute ("DataRate", StringValue ("10"+rateUnits));
  	//p2p_hostToEdge.SetChannelAttribute ("Delay", StringValue ("1ns"));
...
//	
	PointToPointHelper p2p_edgeToAgg;
  	...
  	//p2p_edgeToAgg.SetChannelAttribute ("Delay", StringValue ("1ns"));
...
//	
	...
	//
	double target_lambda = 0.8;
	uint64_t max_lambda_agg = 1440 * pow(10,9); // 1440Gbps
	double target_lambda_agg = target_lambda * max_lambda_agg; // 80% load (Gbps)
	double lambda_i = target_lambda_agg / num_flows; // local rate (Gbps)
	double interarrival_time_i = (1 / lambda_i) * pow(10,9); // interarrival time in nanoseconds
	...
	for (i = 0; i < num_edge; i++)
	{
		...
		// Create independent Exponential RV for local Poisson Process on this host
			...
			expRv->SetAttribute ("Mean", DoubleValue (interarrival_time_i));

		// Initialize Starting Time for 1st app on this host
			uint64_t next_app_start_time = (uint64_t)(expRv->GetValue ());
			//next_app_start_time += rand() % max(144, num_apps_per_host) + 0;
			next_app_start_time += rand() % 144 + 0;
		...
		// Create applications on this host
			...
			// Set Approximate Poisson Arrival Start Time for next Flow on this host
				...
				//uint64_t max_rate_at_target_load = (uint64_t)(target_lambda * 10 * pow(10,9)); // lambda * 10Gbps
				uint64_t max_edge_rate_at_target_load = (uint64_t)(target_lambda * 10 * pow(10,9)); // lambda * 10Gbps
				uint64_t max_agg_rate_at_target_load = (uint64_t)(target_lambda * 40 * pow(10,9)); // lambda * 40Gbps
				//double estimated_duration = ((bytesToSend * 8.0 / max_rate_at_target_load)) * pow(10,9);
				double flowsize_bits = bytesToSend * 8.0;
				double estimated_duration = (2.0*flowsize_bits/max_edge_rate_at_target_load + 2.0*flowsize_bits/max_agg_rate_at_target_load) * pow(10,9);
				//next_app_start_time += (uint64_t)(expRv->GetValue ()) + (2.0*estimated_duration);
				next_app_start_time += (uint64_t)(expRv->GetValue ()) + estimated_duration;
				//next_app_start_time += rand() % max(144, num_apps_per_host) + 0; // jitter
				next_app_start_time += rand() % 144 + 0; // jitter
